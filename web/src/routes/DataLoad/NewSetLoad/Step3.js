...
class Step3 extends PureComponent {
  ...
  onEditAttr = (id) => {
    ...
    this.getEntAttrOperation(need, id);
  };
  ...
  getEntAttrOperation = (need, attrId) => {   
    this.props.dispatch({
      type:'form/select', 
      payload:{
        key: 'TGT_FIELD_NM',
  ...
  render() {
    ...
    const { entModal, fieldModal} = modal;
    ...
    const formConfig = [
      {
        span: 24,
        key: 'ENTITY_CN_NM',
        initialValue: this.setInitialValue('ENTITY_CN_NM'),
        rules: [
          {
            required: true,
            message: '实体名称不能为空',
        ...
        itemConfig: {
          ...
          options: selectInfo['ENTITY_CN_NM'],
        },
      },
      {
        span: 24,
      ...
      },
      {
        span: 24,
        key: 'UPDATE_RULE',
        initialValue: this.setInitialValue('UPDATE_RULE'),
        style: this.formItemLayout,
        rules: [],
        label: '更新规则',
        itemType: 'select',
        itemConfig: {
          placeholder: '请选择一个更新规则',
          onChange: this.setUpdate,
          options:[
            {
              key: '01',
    ...
    const prevStep = () => {
      dispatch(routerRedux.push({pathname:'/data-load/set-add/clean', state:{id:this.taskId}}));
    }
    ...
    const saveEnt = () => {
      ...
      validateFields(['ENTITY_CN_NM', 'PK', 'UPDATE_RULE'], (err, values) => {
        if (!err) {
          this.modalDisabled = true;
          ...
          if(this.isUpdate) {
            ...
            this.getAttrData();
            this.isUpdate = false;
            this.entNm = values.ENTITY_CN_NM;
          } else {
            this.attrId = 'new';
            dispatch({
              type: 'form/clean',
              payload : ['SRC_FIELD_NM','TGT_FIELD_NM', 'LOAD_RULE'],
            });
            const need = { type: 'modal/show', payload: 'fieldModal' };
            this.getEntAttrOperation(need);
          } 
        }
      });
    };
    ...
    const saveAttr = () => {
      ...
      validateFields(['SRC_FIELD_NM', 'TGT_FIELD_NM', 'LOAD_RULE'], (err, values) => {
        if (!err) {
          this.modalDisabled = true;
          values.TASK_ID = this.taskId;
          values.LOAD_TASK_ID = this.entId;
          values.BELONG_TYPE = '01';
          const need = [
            {
              type: 'modal/hide',
              payload: 'fieldModal',
    ...
    return (
      <div className={styles.stepTable} >
        <Button onClick={() => showModal('entModal')} type="dashed" style={{ width: '100%', marginBottom: 8 }} icon="plus">
          添加实体映射
        </Button>
        ...
        >
          ...
          <Button onClick={() => saveEnt('fieldModal')} type="dashed" style={{ width: '100%', marginBottom: 8 }} icon="plus">
